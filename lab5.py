...
def RealizarRutina(Puntos,N,mun,t):
    PuntosTrayectoria=CompletoPuntos(Puntos,N)
    C1=ConvPosArr2QArr(PuntosTrayectoria[:,:])
    [r,c]=C1.shape
    for i in range(r):
        rospy.sleep(2.5*t)
        joint_publisher(np.concatenate(((C1[i,:]+AngHomeRad[0:c]),np.array([mun])), axis=0),t)
...
if __name__ == '__main__':
    try:
        ...
        #RutHome(AngHomeRad,1)

        Rut2p(posHome,1,0*np.pi/180)
        ...
        while(caso!=8):
            ...
            if caso==1: 
                ...
                #ArrPos=[PuntosTri[0,:],PuntosTri[1,:],PuntosTri[2,:],PuntosTri[3,:],PuntosTri[4,:]]
